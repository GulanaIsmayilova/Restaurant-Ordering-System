...
class KitchenControllerIntegrationTest {
    ...
    void updateOrderStatus_ShouldReturnBadRequest_WhenInvalidStatus() throws Exception {
        mockMvc.perform(put("/api/kitchen/orders/{id}/status", order.getId())
    ...
    void getActiveOrders_ShouldReturnForbidden_WhenNotAuthorized() throws Exception {
        mockMvc.perform(get("/api/kitchen/orders"))
    ...
    @Test
    @WithMockUser(roles = "WAITER")
    void updateOrderStatus_ShouldReturnForbidden_WhenNotAuthorized() throws Exception {
        mockMvc.perform(put("/api/kitchen/orders/{id}/status", order.getId())
                .contentType(MediaType.APPLICATION_JSON)
